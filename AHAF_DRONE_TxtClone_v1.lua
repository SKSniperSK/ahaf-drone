function(allstates, event, ...)
    ...
    if prefix == my_prefix then
        ...
        -- system call
        if di.type == "s" then
            -- react to version check call
            if di.msg == "vCheck" then

                -- set/find channel for version check reply
                local channel
                if di.channel then
                    channel = di.channel
                else
                ...
                -- create drone information table for version check result
                local vCheckResult = {
                    type = "s",
                    msg = "vCheckResult",
                    player = GetUnitName("player"),
                    version = aura_env.version
                }

                -- serialize drone information table and send version check result back to master
                aura_env.drone_message = Serializer:Serialize(vCheckResult)
                SendAddonMessage(aura_env.my_prefix, aura_env.drone_message, aura_env.drone_channel, sender)
            end
            -- react to hide call
            if di.msg == "hide" then
            ...
            return true
        -- text call (primary function of this drone child aura)
        elseif di.type == "t" then
